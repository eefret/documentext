...
import com.okorana.documentext.DocumenText;
...
//    };
        ...
        if (!Utils.isEmailAddress(number)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164,
                        DocumenText.getApplication().getCurrentCountryIso());
            }
        }
    ...
    public synchronized void setNumber(String number) {
        if (!Utils.isEmailAddress(number)) {
            try {
                mNumber = PhoneNumberUtil.getInstance().format(PhoneNumberUtil.getInstance().parse(number,
                        DocumenText.getApplication().getCurrentCountryIso()), PhoneNumberUtil.PhoneNumberFormat.E164);
    ...
    public synchronized boolean isEmail() {
        return Utils.isEmailAddress(mNumber);
    }
    ...
    private static class ContactsCache {
        ...
        private Contact getContactInfo(Contact c) {
            ...
            } else if (Utils.isEmailAddress(c.mNumber)) {
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            String normalizedNumber = Utils.normalizeNumber(number);
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                try {
                    numberE164 = PhoneNumberUtil.getInstance().format(PhoneNumberUtil.getInstance().parse(number,
                            DocumenText.getApplication().getCurrentCountryIso()), PhoneNumberUtil.PhoneNumberFormat.E164);
                } catch (NumberParseException e) {
                    e.printStackTrace();
                }
        ...
        private Contact internalGet(String numberOrEmail, boolean isMe) {
            synchronized (ContactsCache.this) {
                ...
                final boolean isNotRegularPhoneNumber = isMe || Utils.isEmailAddress(numberOrEmail) ||
                        Utils.isAlias(numberOrEmail);
